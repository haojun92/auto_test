...
def test(network_name: NetworkName, current: int):
    logger.info("进入机制节能模式")
    PCan()[network_name].tx[FrameID(0x485)] = {
        "interval": 100,
        "data": bytes([0, 0, 0, 16, 0, 0, 0, 0]),
        "callbacks": [callback_tx_print],
    }
    time.sleep(100)
    logger.info("退出机制节能模式")
    PCan()[network_name].tx[FrameID(0x485)]["data"] = bytes([0, 0, 0, 0, 0, 0, 0, 0])
    time.sleep(20)
    logger.info("检查")
