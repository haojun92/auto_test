...
def paint(data: dict[str, float]):
    ...
    plt.title("TDA4 Memory")
    plt.scatter([*range(len(data))], [*data.values()], s=1)
    plt.gca().yaxis.set_major_formatter(ticker.FormatStrFormatter("%dM"))
    plt.grid()
...
def main(log_path: str):
    d = {}
    for log_file in Path(log_path).parent.glob("memory*.log"):
...
@click.option(
    "--log-path",
    type=str,
    default=(Path(__file__).parent / "log/qh01/stability/tda4/memory.log").as_posix(),
    help="日志存储路径",
)
