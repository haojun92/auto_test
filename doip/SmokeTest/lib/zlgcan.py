...
class ZCAN(object):
    def __init__(self):
        if platform.system() == "Windows":
            self.__dll = windll.LoadLibrary("./lib/zlgcan.dll")
        else:
    ...
    def OpenDevice(self, device_type, device_index, reserved):
        try:
            self.device_handle = self.__dll.ZCAN_OpenDevice(device_type, device_index, reserved)
        except:
    ...
    def canfd_start(self, device_handle, chn):
        ret = self.ZCAN_SetValue(device_handle, str(chn) + "/canfd_standard", "0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d CANFD standard failed!" %(chn))
        ret = self.ZCAN_SetValue(device_handle, str(chn) + "/initenal_resistance", "1".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Open CH%d resistance failed!" %(chn))
            exit(0)

        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/canfd_abit_baud_rate","500000".encode("utf-8"))  #设置波特率
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/canfd_dbit_baud_rate","2000000".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d baud failed!" %(chn))      
            exit(0)
            
        ret = self.ZCAN_SetValue(device_handle, "0/set_cn","A001".encode("utf-8"))
        if ret == ZCAN_STATUS_OK:
            t = self.ZCAN_GetValue(device_handle, "0/get_cn/1")
            # print(c_char_p(t).value.decode("utf-8"))

        chn_init_cfg = ZCAN_CHANNEL_INIT_CONFIG()
        chn_init_cfg.can_type = ZCAN_TYPE_CANFD
        chn_init_cfg.config.canfd.mode  = 0
        chn_handle = self.InitCAN(device_handle, chn, chn_init_cfg)
        if chn_handle ==0:
            print("initCAN%d failed!" %(chn)) 
            exit(0)
    
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_clear","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_clear failed!" %(chn))
            exit(0)
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_mode","0".encode("utf-8"))    #标准帧滤波
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_mode failed!" %(chn)) 
            exit(0)
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_start","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_start failed!" %(chn))  
            exit(0)        
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_end","0x7FF".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_end failed!" %(chn)) 
            exit(0)
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_mode","1".encode("utf-8"))    #扩展帧滤波
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_mode failed!" %(chn))
            exit(0)
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_start","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_start failed!" %(chn))
            exit(0)        
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_end","0x1FFFFFFF".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_end failed!" %(chn))
            exit(0)
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/filter_ack","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
            print("Set CH%d  filter_ack failed!" %(chn))
            exit(0)

        ret=self.StartCAN(chn_handle)
        if ret != ZCAN_STATUS_OK:
            print("startCAN%d failed!" %(chn))
            exit(0)  

        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/clear_auto_send","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
    ...
    def auto_start(self, device_handle, chn_handle, can_id, dlc, period, data):  

        chn = 0
        ret=self.ResetCAN(chn_handle)
        if ret==1:
        ...
        AutoCAN_B.index                     = 1
        AutoCAN_B.interval                  = period  #ms
        AutoCAN_B.obj.frame.can_id          = can_id
        AutoCAN_B.obj.transmit_type         = 0
        AutoCAN_B.obj.frame.eff             = 0
        AutoCAN_B.obj.frame.rtr             = 0
        AutoCAN_B.obj.frame.len         = dlc
        for j in range(AutoCAN_B.obj.frame.len):
            AutoCAN_B.obj.frame.data[j] = data[j]
        ...
        ret = self.ZCAN_SetValue(device_handle, str(chn) + "/auto_send_canfd", byref(AutoCAN_B))         #添加B canfd报文定时
        ret = self.ZCAN_SetValue(device_handle,str(chn)+"/apply_auto_send","0".encode("utf-8"))
        if ret != ZCAN_STATUS_OK:
    ...
    def can_send(self, chn_handle, can_id, dlc, data):        
        #Send CANFD Messages
        ...
        for i in range(transmit_canfd_num):
            ...
            canfd_msgs[i].frame.len     = dlc
            for j in range(canfd_msgs[i].frame.len):
                canfd_msgs[i].frame.data[j] = data[j]
        ret = self.TransmitFD(chn_handle, canfd_msgs, transmit_canfd_num)
        # print("Tranmit CANFD Num: %d." % ret)

    def can_receive(self,chn_handle):
        #Receive Messages
        rcv_msg = []
        rcv_canfd_msgs = []
        rcv_num = self.GetReceiveNum(chn_handle, ZCAN_TYPE_CAN)
        rcv_canfd_num = self.GetReceiveNum(chn_handle, ZCAN_TYPE_CANFD)
        if rcv_num:
            rcv_msg, rcv_num = self.Receive(chn_handle, rcv_num)
        ...
        if rcv_canfd_num:
            rcv_canfd_msgs, rcv_canfd_num = self.ReceiveFD(chn_handle, rcv_canfd_num)
        ...
        return rcv_msg, rcv_canfd_msgs

    def open_device(self):
        handle = self.OpenDevice(ZCAN_USBCANFD_200U, 0, 0)
        if handle == INVALID_DEVICE_HANDLE:
            print("Open CANFD Device failed!")
            exit(0)
        print("device handle:%d" %(handle))
        info = self.GetDeviceInf(handle)
